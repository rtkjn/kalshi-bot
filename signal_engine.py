...
Entry signal fires when:
  ...
  2. YES or NO odds are between ENTRY_ODDS_FLOOR and ENTRY_ODDS_THRESHOLD
     e.g. between 35% and 40% — avoids trading heavily one-sided markets
...
Exit signal fires when:
  - An open position's odds recover to EXIT_ODDS_LOW or above
...
class SignalEngine:
    ...
    def evaluate_entry(self, market: dict, open_tickers: Set[str]) -> Signal:
        ...
        threshold = self.config.entry_odds_threshold  # e.g. 0.40 — upper bound
        floor     = self.config.entry_odds_floor      # e.g. 0.35 — lower bound

        # Only enter if odds are in the 35-40% range — genuine dislocation,
        # not a market that has strongly priced in one outcome already
        if yes_price > 0 and floor <= yes_price < threshold:
            return Signal(SignalType.ENTRY, ticker, "yes", yes_price, asset,
                          f"YES odds {yes_price:.2%} < threshold {threshold:.2%}")

        if no_price > 0 and floor <= no_price < threshold:
            return Signal(SignalType.ENTRY, ticker, "no", no_price, asset,
                          f"NO odds {no_price:.2%} < threshold {threshold:.2%}")

        return Signal(SignalType.NONE, ticker, "", 0, asset, "odds not in entry range")
